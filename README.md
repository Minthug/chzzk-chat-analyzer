...
- **포인트 스파이크 감지**: 통계 기반 알고리즘으로 채팅 급증 구간 자동 감지
- **키워드 스파이크 감지**: 사용자 지정 키워드의 급증 구간 별도 감지
- **매니저 채팅 수집**: 매니저 채팅을 별도로 수집해 팝업 매니저 탭에서 확인
- **Top 3 하이라이트**: 감지 점수 기준 상위 3개 급증 구간에 ★ 표시 (시간순 정렬 유지)
...
- **자동 내보내기**: 세션 초기화 전 급증 구간이 있으면 TXT 파일 자동 저장 (설정에서 ON/OFF 가능)
...
- 상대적 급증 감지 방식이라 해당 키워드가 평소에 자주 나와도 **상대적 급증**만 감지
- 방송 접속 초기에는 베이스라인 수집 중으로 키워드 스파이크가 감지되지 않으며, 충분한 데이터가 쌓인 후 감지 시작 (오탐 방지)
...
급증 구간이 많을 때 감지 점수 기준 상위 3개에 ★ 표시로 핵심 포인트를 빠르게 파악할 수 있습니다.
...
영상이 끝나거나 다른 영상으로 이동할 때 세션이 자동으로 처리됩니다.

| 상황 | 동작 |
|---|---|
| 다른 VOD / 라이브로 이동 | 이전 세션 자동 내보내기 → **데이터 보존** (돌아왔을 때 스파이크 복원) |
...
- **설정에서 자동 저장 ON/OFF 가능** — 꺼도 TXT/CSV 버튼으로 수동 저장은 항상 가능
...
낮을수록 작은 변화도 급증으로 감지하고, 높을수록 확실하게 튀는 구간만 잡습니다.
...
| 항목 | 기본값 | 설명 |
|---|---|---|
| 썸네일 자동 캡처 | ON | 급증 구간 감지 시 화면 자동 캡처 |
| TXT 자동 저장 | ON | 방송 이동/종료 시 TXT 파일 자동 다운로드 |
...
- 스토리지 용량 초과 시 썸네일만 자동 삭제 (급증 구간 데이터는 유지)
...
볼륨 스파이크와 키워드 스파이크 모두 동일한 알고리즘으로 감지합니다.
...
| 버전 | 주요 변경 |
|---|---|
| **v1.9.4** | 분석기 미동작 시 자동 새로고침 (tryMount 15회 실패 → live/vod 페이지 자동 복구), VOD 파형 그래프 소실 수정 (React 리마운트 시 캔버스 즉시 재마운트) |
| **v1.9.3** | VOD 스파이크 감지 완전 수정 (채팅 일괄 로드 시 wall-clock 대신 영상 타임스탬프 기준 윈도우 진행), 페이지 이동 시 분석 데이터 소실 방지 (이동 직전 in-memory → storage 즉시 반영), 일시중지 상태 읽기 오류 수정 (sync→local) |
| **v1.9.2** | 설정 용어 개선 (Z-Score 임계값 → 감지 민감도, 윈도우 크기 → 집계 주기), 감지 민감도 설명 문구 직관화, 스파이크 표시 집계 주기 하드코딩 수정 (설정값 반영), 희귀 키워드 감지 불가 버그 수정, 설정 저장소 안정화 (sync→local 전환으로 채널 이동 시 키워드 소실 방지) |
| **v1.9.1** | 채팅 수집 일시중지/재개 버튼 추가, VOD 급증·키워드 감지 오류 수정 (윈도우 진행을 wall-clock 기반으로 통일하여 videoTimestamp 미취득 시에도 정상 감지) |
| **v1.9.0** | 파일명 버그 수정 (스트리머명 대신 pageId로 저장되던 문제), 다른 방송 이동 후 복귀 시 스파이크 사라지던 문제 수정 (이동 시 데이터 보존), 키워드 스파이크만 있을 때 내보내기 버튼 비활성화 버그 수정, 방송 이동 시 이전 채팅 버퍼가 새 방송에 섞이던 문제 수정, TXT 자동 저장 ON/OFF 설정 추가, MIT 라이선스 추가 |
| **v1.8.0** | 키워드 스파이크 오탐 수정 (초기 베이스라인 부족 시 잘못된 감지 방지), 라이브 타임스탬프 보정 레이스 컨디션 수정, 라이브 방송 경과 시간 지속 동기화 |
| **v1.7.0** | 라이브 방송 타임스탬프 정확도 개선 (chzzk API 방송 시작 시각 기반), 썸네일 캡처 권한 수정, 스토리지 쿼터 초과 시 썸네일 삭제 루프 버그 수정, 개인정보처리방침 페이지 추가 |
