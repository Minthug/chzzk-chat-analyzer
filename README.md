...
데이터 손실을 방지하기 위해 **TXT 파일을 자동으로 다운로드**합니다.
...
| 버전 | 주요 변경 |
|---|---|
| **v1.9.0** | 파일명 버그 수정 (스트리머명 대신 pageId로 저장되던 문제), 다른 방송 이동 후 복귀 시 스파이크 사라지던 문제 수정 (이동 시 데이터 보존), 키워드 스파이크만 있을 때 내보내기 버튼 비활성화 버그 수정, 방송 이동 시 이전 채팅 버퍼가 새 방송에 섞이던 문제 수정 |
